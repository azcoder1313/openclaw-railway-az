...
(function initCoreProduct() {
  try {
    ...
    // 2. Patch AGENTS.md with full inline V3 script
    ...
    // 4. Clear ALL sessions to force fresh bootstraps on next message
    try {
      ...
      if (fs.existsSync(sessionsDir)) {
        ...
        files.forEach(function(file) {
          if (file.endsWith(".jsonl")) {
            try { fs.unlinkSync(sessionsDir + "/" + file); cleared++; } catch (_) {}
          }
        });
        console.log("[init] Cleared " + cleared + " sessions");
      }
    } catch (sessErr) {
